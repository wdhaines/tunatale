...
## Layer 81 — PENDING listen grades are hidden from the review badge and the served main queue (deliberate TT-only divergence)
...
**Mirror points touched (both, or badge and queue disagree).** `count_review_due_collocations` gains `AND cd.collocation_id NOT IN (SELECT collocation_id FROM pending_listen_grades)`; `_compute_live_main` filters `due` by `db.pending_grade_ids()`. The filter is **unconditional** — it is not a bury, so it does not ride on `bury_review`. Removing pending cards before the `[:review_remaining]` slice also means a pending card does not consume a review slot; the budget is charged when the grade is actually applied.

**The non-obvious half: pending collocations still seed the sibling-bury seen-set** (`set(learning_collocation_ids) | pending`). Anki WOULD have gathered that review card and buried its NEW sibling. Dropping the card from `due` without seeding `seen` would leak the exclusion into Layer 64's new-card arithmetic and surface a new card that `count_new_available_collocations` (deliberately NOT given a pending clause) still hides — a badge/queue split. Pinned by `test_pending_grade_exclusion.py::test_a_new_sibling_of_a_pending_card_stays_buried`, sabotage-drilled.

**Scope.** Review pool only. LEARNING-state cards are also staged by a listen but keep serving from the learning queue and the learning badge; grading one there applies + clears its pending row like any other real grade, so it cannot double-apply. `count_new_available_collocations` is untouched: `_listen_grade_class` returns None for NEW, so a listen never stages a NEW direction and there is nothing to exclude.

**Release paths (all clear the row).** Per-card `drill_feedback` — on ANY real grade, not just the `lesson_review` one, or a main-queue grade would leave the row behind and hide the card forever; the revlog kind is re-derived from dueness at release time (`_release_review_kind`), never read off the row's stored `grade_class`, because a sync or rollover in between can have moved `due_at`. Bulk `POST /lesson/{id}/commit-pending`. And `sync_pull`, when Anki graded the card instead — gated on `_is_anki_grade` (reps up, or `last_review` moved forward), NOT on `_direction_differs`, which also fires on bury flips and `mod` bumps that are not reviews.

**Files.** `app/srs/db_counts.py`, `app/srs/anki_mirror/queue_engine.py`, `app/api/srs.py`, `app/plugins/anki_sync/sync_engine.py`. Tests: `test_pending_grade_exclusion.py`, `test_pending_grade_apply.py`, `test_api_commit_pending.py`, `test_pending_grade_sync_clear.py`, `test_lesson_queue_pending.py`.

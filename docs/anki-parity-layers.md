...
## Layer 81 — PENDING listen grades hidden from the review badge and served main queue — **INTRODUCED 2026-07, RETIRED 2026-08-10 (F-14)**

**⚠️ Read the retirement before acting on anything below it.** This Layer is kept, not deleted, because it is the one entry in this document that records a divergence TT opened *on purpose* and then closed again. A future session that finds pending cards in the main queue must not "restore" the filter.

### The retirement (F-14, user decision 2026-08-05, shipped 2026-08-10)

> *"I'm not convinced that the TT behavior where it withholds staged cards from the review queue makes sense. What harm is it to review the card in the queue, commit it, and remove it from staging at that point if you do it in that order? That way it matches Anki and seems a bit more intuitive to me."*

**A staged card that is DUE is now counted by the review badge and served in the main queue, exactly as Anki counts and serves it.** Grading it in the main flow applies a real grade and releases the staging, in that order. The divergence below is gone; the review badge no longer sits below Anki's by the pending count.

**The original rationale had already gone stale.** "Offering it in the main flow too would have the user grade it twice" described a risk that the release path had since eliminated: `drill_feedback` clears the pending row on ANY real grade, unconditionally. Three mechanisms already made the retirement safe, all verified by inspection before the change and none of them new work:

1. **Any grade releases the staging** — the unconditional `clear_pending_grade` in `drill_feedback`, whose own comment already stated F-14's rationale.
2. **The revlog kind is re-derived from dueness at release time** (`_release_review_kind`), never read off the row's stale stored `grade_class`.
3. **The frozen main queue does not re-serve a released card.** `_compute_live_main` intersects the cached order against a freshly computed live pool on every call, so a card that is no longer due drops out even though the frozen order still names it. This is why the obvious double-question objection does not apply.

**What was removed.** The `NOT IN (SELECT collocation_id FROM pending_listen_grades)` clause in `count_review_due_collocations`; the `db.pending_grade_ids()` filter over `due` in `_compute_live_main`; and — the tell that the exclusion had been load-bearing in the wrong direction — the compensating `| pending` term in the sibling-bury seen-set, which existed *only* to undo a side effect of the filter. With pending cards back in `due`, the `_bury` pass over `nonlearning_due` seeds the seen-set naturally, so Layer 64's new-card arithmetic is unchanged and a NEW sibling of a staged review card still stays buried. **Net deletion, no replacement logic.**

**The accepted cost, recorded because it is real.** Pending cards now consume review slots; the old filter ran before the `[:review_remaining]` slice, so a staged card did not charge the budget until its grade was applied. Measured 2026-08-05: 2 of 134 held rows were due, so the day's effect was 2 slots against a cap of 100. It scales with how many bucket rows have ripened, not with bucket size. Accepted because Anki charges those cards too — this is a parity **gain**.

**Oracle.** `test_pending_grade_inclusion.py` (renamed from `test_pending_grade_exclusion.py`, whose thesis inverted). It pins ABSOLUTE membership against the `pending_listen_grades` table rather than comparing badge to queue — the two mirror each other by construction, so a surface-vs-surface test passes both before and after and proves nothing. Includes the not-due control (a staged card that was never due is absent for the right reason) and the frozen-queue no-second-offer case for mechanism 3.

### The original divergence (historical — no longer in force)

**Why it existed.** Under the pending-bucket model (bd `tunatale-3ue`, closed) a listen STAGES provisional grades into the TT-only `pending_listen_grades` table instead of applying them. FSRS, the revlog, `dirty_fsrs` and sync see nothing. The card was treated as provisionally handled and served in the lesson "Check your work" queue, so it was withheld from the main review flow.

**The divergence.** TT's review badge sat **BELOW** Anki's by the pending count until the user released them. Same family as the documented `promote_to_learning` listen-first addition (`.claude/rules/anki-queue-parity.md` §"learning" caveat). It was self-healing: applying a pending grade cleared the row and the card returned to the pool.

**Scope.** Review pool only. LEARNING-state cards are also staged by a listen but kept serving from the learning queue and the learning badge. `count_new_available_collocations` was deliberately never given a pending clause — `_listen_grade_class` returns None for NEW, so a listen never stages a NEW direction. F-14 makes that untouched case the consistent rule rather than the exception.

**Release paths (all clear the row — these still stand).** Per-card `drill_feedback` on ANY real grade; bulk `POST /lesson/{id}/commit-pending`; and `sync_pull` when Anki graded the card instead — gated on `_is_anki_grade` (reps up, or `last_review` moved forward), NOT on `_direction_differs`, which also fires on bury flips and `mod` bumps that are not reviews.

**Files.** `app/srs/db_counts.py`, `app/srs/anki_mirror/queue_engine.py`, `app/api/srs.py`, `app/plugins/anki_sync/sync_engine.py`. Tests: `test_pending_grade_inclusion.py`, `test_pending_grade_apply.py`, `test_api_commit_pending.py`, `test_pending_grade_sync_clear.py`, `test_lesson_queue_pending.py`.

## Layer 82 — learning steps and FSRS params resolve from the request's db, by injection (per-language isolation of the db-less resolver call sites)

**Bug.** `_get_steps_for_state` (grading) and `_merge_by_retrievability_ascending` (retrievability sort) still called the `resolve_learning_steps` / `resolve_relearning_steps` / `resolve_fsrs_params` resolvers with **no db**, so each self-constructed an `SRSDatabase` from `settings.database_url` — the SINGULAR setting. In multi-language mode the request's db comes from the PLURAL `settings.database_urls[code]` map, so a Norwegian grade read SLOVENE learning steps and the Norwegian queue sorted with SLOVENE FSRS params. Measured pre-fix: per-language db (no) said `([25.0, 55.0], 'cache')`; the grading path called `([1.0, 10.0], 'cache')`.

**Mechanism.** Resolve in the caller that holds the db, then INJECT — exactly how `params` and the Layer-55 live load balancer already reach `schedule()`. `fsrs.py` stays pure scheduling logic (no `SRSDatabase` threaded in).

- `schedule()` gains optional `learn_steps` / `relearn_steps`; the three step helpers (`_schedule_new`, `_schedule_review_again`, `_schedule_with_steps`) use the injected list when given and only fall back to the db-less `_get_steps_for_state` when `None`.
- `_merge_by_retrievability_ascending` gains optional `params`; `_compute_live_main` resolves `resolve_fsrs_params(db)` and passes it (the old db-less call was the other live offender).
- Callers wired: the `/grade` endpoint, the listen `_apply_confirmed` path, and `commit-pending` (all `resolve_learning_steps(db)` + `resolve_relearning_steps(db)` then inject); the previous listen+lesson grades already threaded `params`/`balancer` the same way.

**Tests.** Locked oracle `test_queue_stats_language_isolation.py` (red pre-fix, 7 passed; the two load-bearing tests point `settings.database_url` at an unopenable path so any reintroduced db-less fallback is observable). The oracle pins the SEAM (the callee honours injected values), not the wiring — the grade sites and `_compute_live_main` are verified by inspection. New branch coverage for the RELEARNING + injected-`relearn_steps` path in `test_fsrs_steps.py::test_relearning_uses_injected_relearn_steps`.

**Documented residual.** `db_revlog.rebuild_from_revlog` (revlog replay) still calls `schedule()` without injected steps, so a replay crossing a learning-step transition still reads the singular setting. Same defect class; not one of the two live paths this layer closes, and left for a follow-up rather than expanding this change's blast radius.

## Layer 83 — the new-card limit was applied AFTER the Template sort, turning a ranking into a filter (and the two display-order settings are now mirrored per deck)

**The report (user, 2026-08-21).** "Why haven't I seen a single production direction from these cards yet?" — after 235 production cards had been minted over four days. Measured on the live `tunatale_no.db`: 290 new production cards at positions 1,000,287–1,000,622, 1439 new recognition cards, `daily_new_cap = 3`, every production card `reps=0` / `introduced_at IS NULL`. None had ever been served, in either app.

**The first diagnosis was wrong, and the arithmetic is why it survived scrutiny.** Two mechanisms predict the same ~478 days: "the queue walks 1434 positions at 3/day before reaching position 1,000,287", and "TEMPLATE ranks every ord=0 card ahead of every ord=1 card, so production waits for the recognition pool to empty — 1439/3". Reading TT's own queue engine produces the second story, because that is what TT implemented. Only the real binary separates them, and it contradicted it on the first run: with `new/day=1` and the production card at position 10, Anki served **the production card**, under the default TEMPLATE setting.

**What Anki actually does.** `gather_new_cards` walks new cards in POSITION order and stops at the limit; `sort_new_cards` then applies `NewCardSortOrder` **to what it gathered**. The limit is a position window, and the sort only reorders what is inside it. Pinned three ways in `test_parity_new_card_sort_order.py`: at `new/day=1` the front-positioned production card is the sole served card; at `new/day=3` the window is {10, 1000, 1001} and TEMPLATE serves all three, ranking production last; NO_SORT serves the same three in position order, production first.

**TT's bug.** `_compute_live_main` sorted the **whole** post-bury pool by template and truncated afterwards. Under TEMPLATE that inverts the meaning of the sort: no ord=1 card can survive `[:new_quota]` while any new ord=0 card exists anywhere in the pool, at any position. 284 minted, 0 served, and repositioning them — the fix the original bead proposed — would have changed nothing on TT's side while fixing Anki's. **Fix:** truncate to `new_quota` in gather order first, then apply the template sort to that slice.

**Two hardcoded settings, mirrored.** `new_card_sort_order` (field 32) and `new_card_gather_priority` (field 34) were compile-time constants in TT — TEMPLATE and HighestPosition — cited as parity with Layer 25's "the user flips this once". Reading the collection settled it: the Slovene deck **is** on HighestPosition (34 = 2), exactly as Layer 25 described, and the Norwegian deck's preset is on DECK (34 absent ⇒ 0). It is a **per-deck** setting and a second deck arrived with a different value, so TT walked the Norwegian new pool from the opposite end to Anki. Both fields now mirror through `refresh_review_settings` into `anki_state_cache` like every other ANKI_CONFIG key, defaulting to Anki's own app defaults (TEMPLATE / DECK). Slovene's behaviour is unchanged — it resolves to 2, which is what the old constant hardcoded.

**absent ⇒ default, not "leave the cache alone".** Both writes are unconditional on absence, the tunatale-6kl rule: proto3 omits a scalar at its default, so a `is not None` guard lets a stale non-default survive forever once the user sets the value back. `test_absent_fields_overwrite_a_stale_non_default_cache` seeds a non-default first, which is what makes it discriminating.

**Closed by Layer 84.** Position is the lever and the mint allocator still handed out `MAX(due)+1`, so the 284 existing production cards remain outside the window. Placing TT-minted cards at the FRONT of the position range — production so it drains first, and TT's own `/listen` auto-adds so they keep surfacing immediately — is the remaining work. ⚠️ Under DECK gather `MAX(due)+1` means *last*, which would silently reverse the Layer 24/25 listen-first intent for Norwegian; the NULLS FIRST clause preserves it only until `sync_create_new` allocates a real position.

**Files.** `app/srs/anki_mirror/queue_stats.py` (fields, resolvers, `new_cards_gather_descending` / `new_cards_ranked_by_template`), `app/srs/anki_mirror/cache_registry.py` (two ANKI_CONFIG keys; `session_main_queue` logic_version 1 → 2 per the pre-Layer checklist), `app/srs/db_queue.py` (`get_new_items` position direction), `app/srs/anki_mirror/queue_engine.py` (`_merge_directions` gather key; truncate-then-rank). Tests: `test_parity_new_card_sort_order.py` (oracle), `test_new_card_display_order.py` (TT mirror, case for case), plus the mirror/resolver tests in `test_queue_stats_cache.py` and the rewritten position oracles in `test_srs_database.py` / `test_api_srs.py`.

## Layer 84 — every card TunaTale writes was allocated to the BACK of the new queue (front-of-queue allocator + a reserved production band)

**The other half of Layer 83.** Layer 83 fixed which cards TT *serves*; the cards were still written to the wrong end of the position range. `OfflineWriter` allocated `MAX(due)+1` everywhere — production mints, `/listen` auto-adds, cloze notes — which is the front of the queue only under HighestPosition gather. On the Norwegian deck (DECK gather) it is the very back, behind 1400+ imported words.

**Two allocators, because the two card classes want opposite growth directions.**

- **`_next_front_position(slots)`** — for TunaTale's own additions, whose point is to surface immediately (Layer 24/25). Returns `MAX(due)+1` under HighestPosition and `MIN(due)-slots` under DECK; cards are written at `base + ord` so a note's templates stay contiguous and in ord order. `sync_create_new`'s oldest-first sort is unchanged and now works under *either* direction: each addition lands ahead of the previous one, so the freshest ends up frontmost. **This was a live regression introduced by Layer 83** — before it, TT gathered DESC and fresh adds surfaced first in TT while Anki put them last; after it, both agreed on *last*.
- **`_next_production_position()`** — for minted production cards, which want FIFO so that mint order survives as serve order. Allocating downward from the frontier would serve the most recently minted card first, silently reintroducing the bug `tunatale-qf6.11` exists to fix. Instead a reserved band `[-1_000_000, 0)` fills *upward*, sitting below the imported deck's 1518-and-up. Exhaustion raises rather than handing out a colliding position.

**Negative positions are load-bearing and were verified, not assumed.** `cards.due` is an i32 and no Anki UI offers a negative starting position, so `test_parity_front_positions.py` pins against the binary that the scheduler gathers `[-1_000_000, -999_999, -5, 0, 7, 1518]` in exactly that order, and that a banded card beats a 1518-based deck at `new/day = 3`. An unverified assumption under a position allocator is precisely what produced Layer 83's wrong diagnosis.

**HighestPosition keeps its old behaviour on both paths.** There the tail already *is* the front, so `MAX(due)+1` was right all along and no band is used. The Slovene deck is the one on that setting, and it does not mint production cards in practice — its notes have carried both templates since import. Documented residual: cross-batch mint order there is newest-first.

**The already-minted cards** are moved by `app/plugins/anki_sync/reposition_production_cards.py` (CLI: `scripts/reposition_production_cards.py`), one-shot and idempotent — a second run reports zero moves and does not even bump `col.mod`. Slots are assigned by **`notes.id` ascending**, which is deck/frequency order for the imported deck (`Spearman(note_id, zipf) = -0.701`; card id correlates the *other* way at +0.509 and is not a substitute). It writes `cards.due` only — `usn = -1` and `mod` per row, one `col.mod` bump, never `col.usn`, no `col.scm`, so nothing forces a full sync — inside `safe_open(mode="rw")` with a post-write `ctx.audit_changes` against the backup. It also points TT's `anki_due` mirror at the new values, because otherwise TT serves the old order until the next `sync_pull` and the change reads as a no-op. It refuses to run against a deck that gathers descending, where it would move cards to the back.

⚠️ **The population is 158, not the 290 `tunatale-uze6` measured**, and the dry run against the real collection is what surfaced that. 132 of TunaTale's "minted" production directions point at Anki cards that do not exist — an empty `Image` field, no production card, and no `graves` row, which together say the collection half of those mints never landed while TunaTale recorded `anki_card_id`/`anki_due` anyway. Independent of these layers, filed as `tunatale-qf6.14`, and untouched by the migration (it only moves cards that exist, and matches the mirror on `anki_card_id`). Do not "reconcile" the two numbers by widening the migration's selection — the missing cards are not mispositioned, they are absent.

**Files.** `app/plugins/anki_sync/sync_writer.py` (band constants, both allocators, `gather_descending` on the writer), `app/plugins/anki_sync/sync.py` (wiring), `app/plugins/anki_sync/sync_engine.py` (sort comment), `app/plugins/anki_sync/reposition_production_cards.py`, `scripts/reposition_production_cards.py`. Tests: `test_parity_front_positions.py` (oracle), `test_anki_reposition_production_cards.py`, plus the rewritten position oracles in `test_anki_production_mint.py` and `test_anki_sync_create_new.py` — each paired with a HighestPosition mirror-image case, so the writer is proven to read the setting rather than default.

## Layer 85 — a seeded starter card is a REVIEW card with no reps, and "has a schedule" stopped meaning `reps > 0`

**Where it comes from.** Cebuano starter cards (`tunatale-u8nz.7`, `app/srs/cognate_seed.py`) begin as review cards carrying stability the learner earned in Tagalog. Nothing was studied in Cebuano, so `SRSDatabase.seed_review_state` writes no revlog row (a fabricated review would be trained on by Anki's FSRS optimizer as a real answer), leaves `reps = 0` and `introduced_at` NULL, and pushes through the existing force path: `set_due_date` (queue 2, type 2), `cards.data` s/d/lrt/dr, `ivl = round(s)`.

**The bug the round-trip oracle caught in the first design.** It left `last_review` NULL and let Anki date the card by `due - ivl`. Anki does exactly that for retrievability, but TT's reader returns no `last_review` for a `reps == 0` card unless `lrt` is present (`parse_fsrs_data`), so after the pull TT still had NULL and `compute_retrievability` answered `desired_retention` — the card sorted at R = 0.9 in TT and at its real R in Anki. And the oracle harness records that Anki's `next_states` treats a card with no `lrt` as answered moments ago, which would have applied SHORT-TERM stability to the first real grade. **Fix:** the seed writes `last_review` = the review its schedule implies, one interval before the due day at the seeding's time of day (inside the right Anki day in every zone, never midnight UTC, which TT would read as a day-level marker), and the push carries it as `lrt`.

**Two restore sites read `reps > 0` as "has a schedule".** The daily unbury sweep (`db_queue.unbury_if_needed`) and unsuspend (`db_directions.set_suspended`) restored a `reps == 0` card to NEW. Anki restores from the card's `type`, and a seeded card's type is review — so TT would have served it as a new card until the next pull, and grading it there would have discarded the head start. Both now restore REVIEW when `reps > 0 OR last_review IS NOT NULL`. The one other population this touches is `promote_to_learning` cards (reps 0, last_review stamped), whose Anki side `set_due_date` already made type 2, so REVIEW is Anki's answer for them too.

**Files.** `app/srs/db_directions.py` (`seed_review_state`, `set_suspended`), `app/srs/db_queue.py` (`unbury_if_needed`), `app/srs/cognate_seed.py`, `scripts/seed_cognate_cards.py`. Tests: `test_seed_review_state.py` (the push+pull round trip against a real collection file is the oracle: queue 2, due on the seeded day, `lrt` present, no revlog, sibling untouched, no recompute divergence, TT R in (0.9, 1)), `test_cognate_seed.py`, `test_seed_cognate_cards_script.py`.

...
- **Function words** — per-language JSON at `backend/app/plugins/languages/<code>/data/function_words.json`, loaded via the registry (`get_function_words_path()`); unlocks cloze cards for grammar-bearing words. `backend/scripts/build_function_word_list.py` can bootstrap a corpus-derived list.

...
- **`lemma_table_path`** (optional) — a gzipped `surface, upos, lemma, is_default` table that reproduces the language's model without PyTorch. Production runs `LEMMATIZER_TYPE=table`, which serves it (`app/srs/lemma_table.py`); a language without one stays lowercase there. Norwegian's is built from Stanza by `backend/scripts/build_stanza_lemma_table.py`, and its ambiguous words (~7% of tokens) are resolved at publish time by one batched LLM call per lesson that picks a TAG from the table's own readings (`app/srs/lemma_resolver.py`, tunatale-kbb.18). The table works because Stanza's lemmatizer is a function of `(word, UPOS)` only.
- **`vocab_notetype`** — the TT-managed Anki notetype cards are minted into, defined in `backend/app/cards/vocab_notetype.py` (`SLOVENE_VOCAB`, `NORWEGIAN_VOCAB` are the references). Adding a notetype bumps `col.scm` — read `.claude/rules/anki-sync.md` first.

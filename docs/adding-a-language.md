...
Adding a language means adding **one entry to `_CONFIGS` in `backend/app/languages.py`** plus the per-language assets that entry points at. There is no other switchboard: `app/main.py` resolves everything through the registry accessors (`get_language` / `get_preprocessor` / `get_deck_name` / `get_tts_voice` / `get_lemmatizer_type` / `get_syllabifier` / `get_vocab_notetype` / `uses_compound_word_breakdown` / `get_variant_separator` / `get_morphology_profile`), and the sync/render paths bundle it all via `resolve_language_context(code, settings) → LanguageContext`.
...
| 3 | `TextPreprocessor` | `backend/app/audio/preprocessing/<code>.py` | One-method protocol (`base.py`); pass-through is a fine start, add EdgeTTS-quirk replacements as you find them by ear |
| 4 | LLM style guide | `backend/app/generation/language_styles/<code>_style.md` | Copy the shape of `sl_style.md` / `no_style.md`: family + confusable-language guards, orthography, LLM failure modes, good/bad examples |
| 5 | Registry entry | `backend/app/languages.py` `_CONFIGS` | The one place that wires 1–4 together, plus `deck_name`, `lemmatizer_type`, `vocab_notetype`, and the optional facets below |
| 6 | Runtime config | `.env` | Multi-language mode: add the code to `settings.database_urls` (per-language TT db); requests select the language via the `X-TT-Language` header. Single-language fallback: `TARGET_LANGUAGE` + `DATABASE_URL` + `ANKI_DECK_NAME` |
...
- **`compound_word_breakdown`** — `True` routes the Pimsleur word breakdown through compound/morpheme-aware segmentation instead of per-syllable buildup. Norwegian is the only user today (`backend/app/generation/norwegian_breakdown.py`: frequency-ranked compound splitting, s-joint/geminate handling, closed-class stem stoplist — design in `docs/bp-brief-segmenter-homographs-overlap.md`). A new compounding language would add its own breakdown module and dispatch via this flag in `section_builder.py`.
- **`lemmatizer_type`** — morphological engine per language (`classla` for Slovene, `stanza` for Norwegian, `lowercase` default). A property of the *language*, not the process: multi-language mode runs both in one process. Heavy engines are opt-in extras (`uv sync --all-groups --extra classla` / `--extra stanza`).
...
- **Function words** — per-language JSON at `backend/app/srs/data/function_words/<code>.json`, loaded by `app/srs/function_words.py`; unlocks cloze cards for grammar-bearing words. `backend/scripts/build_function_word_list.py` can bootstrap a corpus-derived list.
...
`micro-demo-0.0` is the Tagalog audio prototype. Its ~1000-line TTS preprocessor is the long pole for reviving Tagalog; the style guide (`backend/app/generation/language_styles/tl_style.md`) already exists. Touch-points 1, 2, 5, 6 are quick; porting the preprocessor quirks is the work.

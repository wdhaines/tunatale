...
**Status: slice 1 shipped 2026-07-16.** `POST /api/srs/listen` does staged, budget-capped creation and records every listen in `lesson_listens` (server-backed listened store, localStorage migrated). Creation candidates are **corpus-frequency ranked** — wordfreq zipf via the `_rank_listen_candidates` seam, commonest-first, throttled by `daily_new_cap` (2026-08-03; the seam used to rank by raw in-lesson occurrence count, so a lesson-local filler word could outrank a high-utility common word). Listens create individual-word cards only; key-phrase card creation is deferred (phrase quality not there yet). The correction flow exists as the lesson-scoped review session: `/review?lesson={id}` served by `GET /api/srs/lesson/{id}/review-queue` (read-only w.r.t. queue-parity state), linked from the lesson page's "Check your work — review N words". The lesson page also shows aggregate + per-word mastery indicators. Still future from the decisions below: voice-command "I listened", CEFR/grammar-topic pedagogical scoring (word-frequency ranking shipped 2026-08-03; CEFR/grammar-topic scoring has no data yet), and the listening-vs-reading recognition bucket split.
...
- **Creation is throttled into a staged "build."** Never create 50+ cards from one listen. Over multiple listens you gradually acquire (most of) the lesson's vocab. Throttle reuses the existing new-card cap (`daily_new_cap`, queue-parity rule 12) + the word-learning **introduction gates** (`word-learning-state-machine.md`, FSRS Layer 65). *Which* candidates get created is pedagogically ranked — word frequency first, corpus-based (wordfreq zipf, commonest-first, via the `_rank_listen_candidates` seam, 2026-08-03) — not FIFO. *(Frequency tooling: `build_function_word_list.py`, `import_seed`, and the wordfreq ranking. CEFR/grammar-topic gating is likely new.)*
...
### Rewrite prep — the contract the current `POST /api/srs/listen` implementation carries (recorded 2026-07-03, god-module split)

The handler in `backend/app/api/srs.py` is deliberately NOT being refactored ahead of the rewrite — but it already delegates to reusable helpers, so the rewrite swaps orchestration only. Whatever replaces it must keep:

- **The card-adding-UI contract** (`.claude/rules/anki-sync.md` §"When building a new UI that adds cards"): rows via `db.upsert_by_guid()`/`add_collocation()`, `card_type` set on the `SyntacticUnit`, `anki_note_id`/`anki_card_id` left `None`, state `NEW`, `introduced_at` NULL until first grade. Same-day surfacing comes free via the NEW-latecomer tail-append in `assemble_review_queue` (`app/srs/anki_mirror/queue_engine.py`) — don't fight it; only NEW-state cards land mid-session.
- **Grade-eligibility semantics**: `_listen_grade_eligible` (api/srs.py) decides which recognition directions accept the implicit auto-Good. Its rules are the listening-recognition bucket boundary.
- **Reusable helpers that survive the rewrite**: `_persist_new_card` (idempotent create + audio + media), `_generate_add_time_media` (add-time image/word-audio, reused by sync), `_balancer_add` (Layer 55 live load-balancer feed on every grade), and the self-healing sentence-translation backfill for existing rows (memory: `add_collocation` never updates existing rows — new card-adding UIs must mirror /listen's backfill).
- **Idempotency**: re-listening to the same lesson must not duplicate collocations or Anki notes, and re-grades within the same day follow the existing revlog rules.

...
The remaining ~31 within-1h misses across all 89 directions are the **FSRS load balancer** (`loadBalancerEnabled=true` in this collection), **not** `recompute_memory_state` as originally written here — see `docs/anki-parity-layers.md` Layer 53 for the full evidence chain. Drilled findings: (a) TT's `schedule()` is bit-exact with Anki's own scheduler on identical pre-state + `days_elapsed` + fuzz seed — both produce the *same* interval, and both differ from the stored value; (b) every one of the 26 single-grade stored intervals falls *within* TT's computed fuzz range `[lower, upper]` but differs from TT's fuzz pick — the load balancer's signature; (c) a synthetic A/B with heavy load on the pure-fuzz target day shifts the interval 59→61 with the balancer on. The balancer relocates each interval to a less-loaded day inside the fuzz window using the whole collection's due histogram, so a per-card forward-step cannot reproduce the pick. **This does not affect production parity** — `sync_pull` reads `cards.due` directly (the "due_at pass-through from Anki" case), so synced cards carry Anki's load-balanced due verbatim. The `due_at` side-stat is therefore a fuzz-*range* reliability check, not a bit-exact target, and its ceiling is below 100% whenever the balancer is on. Note: unlike Optimize/reschedule, the load balancer fires on **every** grade, so "don't run Optimize during measurement" (below) does not remove it.

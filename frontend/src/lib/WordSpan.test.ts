...
describe('WordSpan', () => {
	it('renders the word surface text', () => {
		const { getByRole } = render(WordSpan, { props: { word: makeWordToken({ surface: 'hvala' }) } });
	...
	it('does not render tooltip when requireModifier=true and altHover=false', () => {
		const { container } = render(WordSpan, {
			props: { word: makeWordToken({ translation: 'hello', srs_state: 'new' }), requireModifier: true, altHover: false }
		});
	...
	it('renders tooltip when requireModifier=true and altHover=true', () => {
		const { container } = render(WordSpan, {
			props: { word: makeWordToken({ translation: 'hello', srs_state: 'new' }), requireModifier: true, altHover: true }
		});

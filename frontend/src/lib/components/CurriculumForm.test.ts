...
describe('CurriculumForm', () => {
	...
	it('enables Generate button when topic is filled', async () => {
		const { getByRole, getByPlaceholderText } = render(CurriculumForm, { props: { onGenerate: vi.fn() } });
	...
	it('shows error message when generateCurriculum fails', async () => {
		...
		const { getByRole, getByPlaceholderText, findByText } = render(CurriculumForm, { props: { onGenerate: vi.fn() } });
		await fireEvent.input(getByPlaceholderText(/ordering coffee/i), { target: { value: 'hiking' } });
		await fireEvent.click(getByRole('button', { name: 'Generate' }));

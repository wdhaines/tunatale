...
describe('review/+page.svelte', () => {
	...
	it('does not surface a learning card the server placed in pending_learning', async () => {
		...
		const oknoPending = makeReviewQueueItem({ id: 1, text: 'okno', direction: 'recognition', state: 'learning' });
		mockSubmitDrill.mockResolvedValue({
			new_due_date: '2026-04-25', new_state: 'learning',
			due_at: new Date(Date.now() + 600_000).toISOString(), left: 1002,
		});
		mockFetchReviewQueue
			.mockResolvedValueOnce({ queue: [item1, item2] })
	...
	it('learning card resurfaces when server promotes it past the cutoff', async () => {
		...
		const item1 = makeReviewQueueItem({ id: 1, text: 'okno', translation: 'window', direction: 'recognition' });
		const item2 = makeReviewQueueItem({ id: 3, text: 'hiša', translation: 'house', direction: 'recognition' });
		const oknoLearning = makeReviewQueueItem({ id: 1, text: 'okno', direction: 'recognition', state: 'learning' });
		mockSubmitDrill
			.mockResolvedValueOnce({
				new_due_date: '2026-04-25', new_state: 'learning',
				due_at: new Date(Date.now() + 60_000).toISOString(), left: 1002,
			})

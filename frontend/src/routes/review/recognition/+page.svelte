<script lang="ts">
	...
	onMount(async () => {
		try {
			queue = await api.fetchDue('recognition');
		} catch (e) {
	...
	async function rate(rating: 'again' | 'hard' | 'good' | 'easy') {
		try {
			await api.submitDrill(current.id, 'recognition', rating);
		} catch (e) {
			error = e instanceof Error ? e.message : String(e);
			return;
		}
...
<main>
	...
	{:else if current}
		...
		<section class="card-section">
			<DrillCard item={current} promptSide="L2" onRate={rate} />
		</section>
	{/if}
</main>

...
# ── Wider strategy limits ──────────────────────────────────────────────────


def test_wider_selects_up_to_8_new(selector):
...
def test_deeper_selects_up_to_3_new(selector):
    new_items = [_make_item(f"word{i}", SRSState.NEW) for i in range(15)]
    review_items = []
...
def test_overdue_items_included_in_review(selector):
    review_items = [
...
def test_select_respects_max_review(selector):
...
def test_score_item_returns_float(selector):
    item = _make_item("test", SRSState.REVIEW, days_overdue=3)
    score = selector.score(item)
    assert isinstance(score, float)


def test_low_stability_item_scores_higher_than_high_stability(selector):

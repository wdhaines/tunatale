...
# ── Initial scheduling (NEW state) ────────────────────────────────────────


def test_schedule_new_good_advances_to_learning():
    item = _new_item()
...
def test_schedule_new_easy_longer_interval_than_good():
    item_good = _new_item()
    item_easy = _new_item()
    today = date.today()
...
def test_schedule_new_again_stays_learning():
    item = _new_item()
    result = schedule(item, Rating.AGAIN)
    assert result.state in (SRSState.LEARNING, SRSState.NEW)


def test_schedule_new_good_stability_above_one():
    item = _new_item()
    result = schedule(item, Rating.GOOD)
    assert result.stability > 0.0


def test_schedule_new_easy_stability_greater_than_good():
    today = date.today()
...
# ── Review scheduling (REVIEW state) ─────────────────────────────────────
...
def test_review_again_triggers_relearning():
    item = _review_item()
    result = schedule(item, Rating.AGAIN)
...
def test_review_again_reduces_stability():
    item = _review_item()
    result = schedule(item, Rating.AGAIN)
    assert result.stability < item.stability


def test_review_good_reps_incremented():
    item = _review_item()
    result = schedule(item, Rating.GOOD)
    assert result.reps == item.reps + 1


def test_review_last_review_updated():
    today = date.today()
    item = _review_item()
...
def test_difficulty_increases_on_again():
    item = _new_item()
    item.difficulty = 5.0
    result = schedule(item, Rating.AGAIN)
...
def test_easy_results_in_lower_difficulty_than_hard():

...
class TestGroundMorphologyFocus:
    """Tests for _ground_morphology_focus — validates/corrects LLM tags with a real lemmatizer."""

    # ── Helpers ──────────────────────────────────────────────────────────

    class _Phrase:
        """Minimal phrase stand-in (matches the fields we access)."""

        def __init__(self, text: str, language_code: str = "sl"):
            self.text = text
            self.language_code = language_code

    def _make_phrases(self, *texts: str) -> list:
        return [self._Phrase(t) for t in texts]

    # ── Confirmed entry kept ─────────────────────────────────────────────

    def test_confirmed_entry_kept(self):
        """Analyzer agrees with LLM → entry kept."""
        from app.api.srs import _ground_morphology_focus
        ...
        morphology_focus = [
            {"surface": "Ljubljani", "lemma": "Ljubljana", "feature": "noun:loc:sg"},
        ]
        phrases = self._make_phrases("Jaz sem v Ljubljani.")

        surface_to_analysis, surface_to_sentence = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        assert "Ljubljani" in surface_to_analysis
        lemma, feature = surface_to_analysis["Ljubljani"]
        assert lemma == "Ljubljana"
        assert feature == "noun:loc:sg"

    def test_wrong_lemma_dropped(self):
        """Analyzer contradicts LLM's lemma → entry dropped."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        # LLM says lemma="dobro" (wrong), analyzer says "dober" (correct)
        morphology_focus = [
            {"surface": "dobro", "lemma": "dobro", "feature": "adj:nom:n:sg"},
        ]
        phrases = self._make_phrases("To je dobro.")

        surface_to_analysis, _ = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        assert "dobro" not in surface_to_analysis

    def test_analyzer_no_feature_keeps_llm(self):
        """Analyzer can't produce A1 feature → keep LLM's entry (tolerant)."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        morphology_focus = [
            {"surface": "mesto", "lemma": "mesto", "feature": "noun:nom:sg"},
        ]
        phrases = self._make_phrases("To je mesto.")

        surface_to_analysis, _ = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        assert "mesto" in surface_to_analysis

    def test_non_a1_feature_skipped(self):
        """Non-A1 feature (e.g., genitive noun) → entry skipped."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer

        stub = StubLemmatizer()
        morphology_focus = [
            {"surface": "mize", "lemma": "miza", "feature": "noun:gen:sg"},
        ]
        phrases = self._make_phrases("Rob mize.")

        surface_to_analysis, _ = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        assert "mize" not in surface_to_analysis

    def test_malformed_entry_skipped(self):
        """Non-dict entry or null fields → skipped (no raise)."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        morphology_focus = [
            ...
            {"surface": "hotelu", "lemma": "hotel", "feature": "noun:loc:sg"},
        ]
        stub.set_sentence(
            "V hotelu.",
            [
        ...
        phrases = self._make_phrases("V hotelu.")

        surface_to_analysis, _ = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        # Only the valid entry should survive
        assert "hotelu" in surface_to_analysis
        assert len(surface_to_analysis) == 1

    def test_surface_not_in_phrases_skipped(self):
        """Surface not found in any NATURAL_SPEED phrase → skipped."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        # Phrase doesn't contain "hotelu"
        phrases = self._make_phrases("To je miza.")

        surface_to_analysis, _ = _ground_morphology_focus(morphology_focus, phrases, stub, "sl")
        assert "hotelu" not in surface_to_analysis

    def test_none_phrases_returns_empty(self):
        """natural_speed_phrases is None → returns empty dicts."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        surface_to_analysis, surface_to_sentence = _ground_morphology_focus(morphology_focus, None, stub, "sl")
        assert surface_to_analysis == {}
        assert surface_to_sentence == {}

    def test_mismatched_language_code_skips_phrase(self):
        """Phrase with wrong language_code → skipped."""
        from app.api.srs import _ground_morphology_focus
        from tests._helpers.lemmatizer import StubLemmatizer
        ...
        # Phrase has language_code "en" while we pass "sl"
        phrases = [self._Phrase("V hotelu.", language_code="en")]

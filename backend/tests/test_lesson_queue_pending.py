...
class TestPendingCardsAreServed:
    ...
    async def test_only_this_lessons_pending_rows_surface_a_rating(self):
        db = _setup(["banka"])
        cid = _track_ahead(db, "banka")
        ...
        # Still served — a pending row anywhere means the card is staged — but
        # the rating belongs to the other lesson's bucket, not this queue's.
        assert [i["text"] for i in served] == ["banka"]
        assert served[0]["pending_rating"] is None

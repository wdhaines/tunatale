...
def test_fallback_creates_log_entry(tmp_path):
    ...
    state = parse_fsrs_data(
        card_id=77, direction=Direction.RECOGNITION, data_str="", queue=0, reps=0, lapses=0, fallback_log_path=log
    )
    assert log.exists()
...
def test_fallback_log_append_only(tmp_path):
    log = tmp_path / "fallback.log"
    parse_fsrs_data(
        card_id=10, direction=Direction.RECOGNITION, data_str="", queue=0, reps=0, lapses=0, fallback_log_path=log
    )
    parse_fsrs_data(
        card_id=20, direction=Direction.RECOGNITION, data_str="", queue=0, reps=0, lapses=0, fallback_log_path=log
    )
...
def test_fallback_log_parent_created_automatically(tmp_path):
    ...
    parse_fsrs_data(
        card_id=1, direction=Direction.RECOGNITION, data_str="", queue=0, reps=0, lapses=0, fallback_log_path=log
    )
    assert log.exists()

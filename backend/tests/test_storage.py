...
def test_save_and_get_curriculum(store):
...
def test_get_curriculum_returns_none_when_missing(store):
    assert store.get_curriculum("nonexistent") is None


def test_list_curricula_returns_all(store):
...
def test_list_curricula_empty(store):
...
def test_save_and_get_lesson(store):
    lesson = _make_lesson()
...
def test_get_lesson_returns_none_when_missing(store):
...
def test_file_based_store_persists(tmp_path):
...
# ── Coexistence with SRSDatabase ──────────────────────────────────────────


def test_shared_db_with_srs_database(tmp_path):

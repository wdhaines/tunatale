...
def build_word_breakdown(phrase_text: str) -> list[str]:
    """Build a progressive word-by-word breakdown ending with the full phrase twice.

    Examples:
        "dan"              → ["dan", "dan"]
        "dober dan"        → ["dan", "dober dan", "dober dan"]
        "eno kavo prosim"  → ["prosim", "kavo prosim", "eno kavo prosim", "eno kavo prosim"]
    """
    words = phrase_text.split()
    steps: list[str] = []
    for i in range(len(words) - 1, 0, -1):
        steps.append(" ".join(words[i:]))
    steps.append(phrase_text)
    steps.append(phrase_text)
    return steps

...
12. **Daily caps limit the served queue, not only the badge (corrected Layer 75).** Anki gathers at most `new_limit - introduced_today` new cards AND `review_limit - reviews_today` review cards into the study session — the limits cap the actual review flow, not just the deck-list badge. `_compute_live_main` mirrors **both**: `nonlearning_new[:new_quota]` and `nonlearning_due[:review_remaining]` (review cap applied *after* sibling-bury, keeping the lowest-R survivors). Learning/relearning cards are **exempt** from the review cap (Anki gathers them regardless). The earlier claim here ("caps are render-only / queue does NOT cap / Anki only caps the badge") was wrong — the new-card cap was always applied in assembly, and a 50-review cap that served 1499 reviews was the bug (user report, 2026-06-28). The freeze model stays consistent: `reviews_today` grows as you grade so the cap tightens, but graded cards leave the due pool, so the surviving frozen reviews always equal the remaining budget (no mid-session drops). Badge and queue now cap at the same `review_cap - reviews_today`.
